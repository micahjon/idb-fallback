...
var idbKeyval$1 = require('idb-keyval');
...
class IdbFallback {
  ...
  initStore(databaseName, objectStoreName, latestVersion, versionKey) {
     ...
     */
    try {
      this.store = new idbKeyval$1.Store(databaseName, objectStoreName);
    } catch (error) {
  ...
  get(key, { useFallback = true } = {}) {
    return this.indexedDBReady.then(useIndexedDB => {
      ...
      return idbKeyval$1.get(key, this.store);
    });
  }
  ...
  set(key, value, { useFallback = true } = {}) {
    return this.indexedDBReady.then(useIndexedDB => {
      ...
      return idbKeyval$1.set(key, value, this.store).then(
  ...
  del(key) {
    return this.indexedDBReady.then(useIndexedDB => {
      ...
      return idbKeyval$1.del(key, this.store);
    });
  }
  ...
  keys() {
    return this.indexedDBReady.then(useIndexedDB => {
      ...
      return idbKeyval$1.keys(this.store).then(keys => {
  ...
  clear() {
    return this.indexedDBReady.then(useIndexedDB => {
      ...
      return idbKeyval$1.clear(this.store);
    });
  }
